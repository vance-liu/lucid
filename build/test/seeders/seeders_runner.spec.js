...
test.group('Seeds Runner', (group) => {
    ...
    test('run a seeder file', async ({ fs, assert, cleanup }) => {
        ...
        await fs.create('database/seeders/User.ts', `export default class FooSeeder {
      ...
      run () {
        ...
        assert.equal(fileSource['invoked'], true);
    ...
    test('catch and return seeder errors', async ({ fs, assert }) => {
        ...
        const db = getDb();
        const runner = new SeedsRunner(db, app);
        await fs.create('database/seeders/User_v1.ts', `export default class FooSeeder {
      run () {
    ...
    test('mark file as ignored when "environment = production" and not running in production mode', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const runner = new SeedsRunner(db, app);
        runner.nodeEnvironment = 'development';
        await fs.create('database/seeders/User_v2.ts', `export default class FooSeeder {
    ...
    test('mark file as ignored when "environment = development" and not running in development mode', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const runner = new SeedsRunner(db, app);
        runner.nodeEnvironment = 'production';
        await fs.create('database/seeders/User_v3.ts', `export default class FooSeeder {
      public static invoked = false
