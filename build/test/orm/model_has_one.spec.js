...
import { getDb, getBaseModel, ormAdapter, setup, cleanup, resetTables, } from '../../test-helpers/index.js';
...
test.group('Model | HasOne | Options', (group) => {
    ...
    test('raise error when localKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.profile" expects "id" to exist on "User" model, but is missing. Did you forget to define the column?');
        }
    });
    test('raise error when foreignKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.profile" expects "userId" to exist on "Profile" model, but is missing. Did you forget to define the column?');
        }
    });
...
test.group('Model | HasOne | preload', (group) => {
    ...
    test('set relationship property value to null when no related rows were found', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
