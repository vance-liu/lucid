...
test.group('Model | Has Many Through | Options', (group) => {
    ...
    test('raise error when localKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "Country.posts" expects "id" to exist on "Country" model, but is missing. Did you forget to define the column?');
        }
    });
    test('raise error when foreignKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "Country.posts" expects "countryId" to exist on "User" model, but is missing. Did you forget to define the column?');
        }
    });
    test('raise error when through local key is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "Country.posts" expects "id" to exist on "User" model, but is missing. Did you forget to define the column?');
        }
    });
    test('raise error when through foreign key is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "Country.posts" expects "userId" to exist on "Post" model, but is missing. Did you forget to define the column?');
        }
    });
...
test.group('Model | Has Many Through | onQuery', (group) => {
    ...
    test('do not invoke onQuery method on related query builder subqueries', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
