...
import { setup, getDb, cleanup, getPosts, ormAdapter, resetTables, getBaseModel, } from '../../test-helpers/index.js';
...
test.group('Model | HasMany | Options', (group) => {
    ...
    test('raise error when localKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.posts" expects "id" to exist on "User" model, but is missing. Did you forget to define the column?');
        }
    });
    test('raise error when foreignKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.posts" expects "userId" to exist on "Post" model, but is missing. Did you forget to define the column?');
        }
    });
    ...
    test('clone relationship instance with options during inheritance', async ({ fs, assert }) => {
        ...
        __decorate([
            hasMany(() => Post, { foreignKey: 'userUid' })
        ], BaseUser.prototype, "posts", void 0);
...
test.group('Model | HasMany | bulk operations', (group) => {
    ...
    test('paginate with additional constraints using related query builder', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | HasMany | sub queries', (group) => {
    ...
    test('run onQuery method when defined', async ({ fs, assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = db
            ...
            .where((query) => query.where('users.id', '=', db.connection().getReadClient().ref('posts.user_id')))
            .toSQL();
        assert.deepEqual(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
    });
...
test.group('Model | HasMany | preload', (group) => {
    ...
    test('set relationship property value to empty array when no related rows have been found', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | HasMany | withCount', (group) => {
    ...
    test('do not set count directly on the model when defined as a getter', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | HasMany | whereHas', (group) => {
    ...
    test('limit rows to the existance of relationship', async ({ fs, assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = connection
            ...
            .where((subquery) => subquery.where('users.id', '=', connection.getReadClient().ref('posts.user_id'))))
            .toSQL();
        assert.deepEqual(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        const users = await query.exec();
