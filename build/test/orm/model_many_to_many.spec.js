...
import { getDb, getBaseModel, ormAdapter, setup, resetTables, cleanup, } from '../../test-helpers/index.js';
...
test.group('Model | ManyToMany | Options', (group) => {
    ...
    test('raise error when localKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.skills" expects "id" to exist on "User" model, but is missing. Did you forget to define the column?');
        }
    });
    ...
    test('raise error when relatedKey is missing', async ({ fs, assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Relation "User.skills" expects "id" to exist on "Skill" model, but is missing. Did you forget to define the column?');
        }
    });
    ...
    test('compute relatedPivotForeignKey from related model name + primary key', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | ManyToMany | preload', (group) => {
    ...
    test('convert dates to luxon datetime instance when preload using model instance', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | ManyToMany | save', (group) => {
    ...
    test('attach duplicates when save is called more than once with with checkExisting = false', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
    ...
    test('attach duplicates with different pivot attributes and with checkExisting = false', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
    ...
    test('attach when related pivot entry exists but for a different parent @sanityCheck', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
...
test.group('Model | ManyToMany | saveMany', (group) => {
    ...
    test('do not attach duplicates when saveMany is called more than once', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
    ...
    test('attach duplicates when saveMany is called more than once with checkExisting = false', async ({ assert, fs, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
