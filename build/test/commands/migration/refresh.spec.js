...
test.group('migration:refresh', (group) => {
    ...
    test('rollback to batch 0 and migrate database', async ({ fs, assert }) => {
        await fs.create('database/migrations/refresh_cmd_users.ts', `
      ...
      `);
        await fs.create('database/migrations/refresh_cmd_posts.ts', `
        import { BaseSchema as Schema } from '../../../../src/schema/main.js'
    ...
    test('run seeders when --seed flag is passed', async ({ fs, assert }) => {
        await fs.create('database/seeders/refres_cmd_user.ts', `export default class UserSeeder {
        public async run () {
          process.env.EXEC_USER_SEEDER = 'true'
        }
      }`);
        await fs.create('database/migrations/refres_cmd_users_v1.ts', `
        import { BaseSchema as Schema } from '../../../../src/schema/main.js'
        export default class User extends Schema {
          public async up () {
            this.schema.createTable('schema_users', (table) => {
      ...
      `);
        await fs.create('database/migrations/posts.ts', `
        import { BaseSchema as Schema } from '../../../../src/schema/main.js'
        export default class Account extends Schema {
          public async up () {
            this.schema.createTable('schema_accounts', (table) => {
