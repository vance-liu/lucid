...
test.group('migrate:fresh', (group) => {
    ...
    test('migration:fresh should drop all tables and run migrations', async ({ fs, assert }) => {
        await fs.create('database/migrations/fresh_cmd_users.ts', `
    ...
    test('migration:fresh --seed should run seeders', async ({ fs, assert }) => {
        await fs.create('database/seeders/fresh_cmd_user.ts', `export default class UserSeeder {
        public async run () {
          process.env.EXEC_USER_SEEDER = 'true'
        }
      }`);
        await fs.create('database/migrations/fresh_cmd_users_v1.ts', `
      import { BaseSchema as Schema } from '../../../../src/schema/main.js'
      export default class User extends Schema {
        public async up () {
          this.schema.createTable('schema_users', (table) => {
