...
test.group('MakeSeeder', (group) => {
    ...
    test('make a seeder', async ({ fs, assert }) => {
        ...
        await assert.fileContains('database/seeders/user_seeder.ts', `import { BaseSeeder } from '@adonisjs/lucid/seeders'`);
        await assert.fileContains('database/seeders/user_seeder.ts', `export default class extends BaseSeeder`);
    });
});
