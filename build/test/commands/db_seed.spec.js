...
test.group('DbSeed', (group) => {
    ...
    test('run seeders', async ({ fs, assert }) => {
        await fs.create('database/seeders/user.ts', `export default class UserSeeder {
        async run () {
    ...
    test('cherry pick files using the --files flag', async ({ fs, assert }) => {
        ...
        await fs.create('database/seeders/post.ts', `export default class PostSeeder {
        public async run () {
    ...
    test('run seeders with compact output', async ({ fs }) => {
        ...
        await fs.create('database/seeders/client.ts', `export default class ClientSeeder {
        public async run () {}
      }`);
        await fs.create('database/seeders/post.ts', `export default class PostSeeder {
