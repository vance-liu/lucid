...
import { setup, getDb, getBaseSchema, createEmitter, cleanup as cleanupTables, } from '../../test-helpers/index.js';
...
test.group('Schema', (group) => {
    ...
    test('get schema queries defined inside the up method in dry run', async ({ fs, cleanup, assert, }) => {
    ...
    test('get schema queries defined inside the down method in dry run', async ({ fs, cleanup, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        cleanup(() => db.manager.closeAll());
    ...
    test('execute up method deferred actions in correct sequence', async ({ fs, cleanup, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        cleanup(() => db.manager.closeAll());
    ...
    test('emit db:query event when schema instructions are executed', async ({ assert, fs, cleanup, }) => {
        assert.plan(10);
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
