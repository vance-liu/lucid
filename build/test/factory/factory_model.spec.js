...
test.group('Factory | Factory Model', (group) => {
    ...
    test('raise exception when state is not registered', async ({ fs, assert }) => {
        ...
        const factory = factoryManager.define(User, () => new User());
        assert.throws(() => factory.getState('active'), 'Cannot apply undefined state "active". Double check the model factory');
    });
    ...
    test('raise exception when relation is not defined', async ({ fs, assert }) => {
        ...
        const factory = factoryManager.define(User, () => new User());
        assert.throws(() => factory.getRelation('profile'), 'Cannot reference "profile" relationship. Make sure to setup the relationship within the factory');
    });
    test('do not allow registering relationships not defined on the model', async ({ fs, assert, }) => {
        const app = new AppFactory().create(fs.baseUrl, () => { });
        await app.init();
        const db = getDb();
        const adapter = ormAdapter(db);
        ...
        const factory = () => factoryManager.define(User, () => new User()).relation('profile', () => { });
        assert.throws(factory, 'Cannot define "profile" relationship. The relationship must exist on the "User" model first');
    });
