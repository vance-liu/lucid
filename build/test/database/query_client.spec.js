...
import { logger, setup, cleanup, getConfig, resetTables, createEmitter, } from '../../test-helpers/index.js';
...
test.group('Query client | read mode', (group) => {
    ...
    test('raise error when attempting to get transaction in read mode', async ({ assert }) => {
        ...
        try {
            ...
            assert.equal(message, 'Write client is not available for query client instantiated in read mode');
        }
        await connection.disconnect();
    });
});
