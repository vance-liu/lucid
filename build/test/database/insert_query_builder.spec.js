...
import { setup, cleanup, getConfig, getQueryClient, getInsertBuilder, getRawQueryBuilder, logger, } from '../../test-helpers/index.js';
test.group('Query Builder | insert', (group) => {
    ...
    test('derive key value from raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .table('users')
            .insert({
            username: getRawQueryBuilder(getQueryClient(connection), `ST_GeomFromText(POINT('row.lat_lng'))`),
        })
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
