...
import { setup, cleanup, getDb, getConfig, getUsers, resetTables, getQueryClient, getQueryBuilder, getInsertBuilder, getRawQueryBuilder, logger, createEmitter, } from '../../test-helpers/index.js';
if (!['better_sqlite', 'sqlite', 'libsql'].includes(process.env.DB)) {
    test.group('Query Builder | client', (group) => {
        ...
        test('use transaction client when insert query is used inside a transaction', async () => {
            ...
            const trx = await client.transaction();
            await new QueryRunner(client, false, null).run(db.table('users').useTransaction(trx).insert({ username: 'virk' }).knexQuery);
            await trx.rollback();
            await connection.disconnect();
        });
        ...
        test('use transaction client when insert query is issued from transaction client', async () => {
            ...
            await new QueryRunner(trx, false, null).run(trx.insertQuery().table('users').insert({ username: 'virk' }).knexQuery);
            await trx.commit();
            await connection.disconnect();
        });
    });
...
test.group('Query Builder | select', (group) => {
    ...
    test('define columns as raw queries', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .select(getQueryClient(connection).raw('(select count(*) as total from addresses) as addresses_total'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | whereIn', (group) => {
    ...
    test('add whereIn as a rawquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereIn('username', getRawQueryBuilder(getQueryClient(connection), `select ${ref('id')} from ${ref('accounts')}`))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .whereIn('username', [connection.client.raw(`select ${ref('id')} from ${ref('accounts')}`)])
    ...
    test('add whereIn as a subquery with array of keys', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereIn(['username', 'email'], getQueryBuilder(getQueryClient(connection)).select('username', 'email').from('accounts'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | whereExists', (group) => {
    ...
    test('wrap subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereExists(getQueryBuilder(getQueryClient(connection))
            .from('accounts')
            .where('status', 'active')
...
test.group('Query Builder | join', (group) => {
    ...
    test('use onJsonPathEquals method from the join callback', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .join('profiles', (builder) => {
            builder.onJsonPathEquals('country_name', // json column in cities
...
test.group('Query Builder | orderBy', (group) => {
    ...
    test('define order by columns as subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .orderBy(getQueryBuilder(getQueryClient(connection))
            .from('user_logins')
...
test.group('Query Builder | union', (group) => {
    ...
    test('define union query as an array of raw queries', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .union([
            getRawQueryBuilder(getQueryClient(connection), 'select * from users where first_name is null'),
        ])
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | having', (group) => {
    ...
    test('add having clause value being a raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .having('user_id', '=', getRawQueryBuilder(getQueryClient(connection), `(select ${ref('user_id')} from ${ref('accounts')})`))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
    ...
    test('add having clause value being a sub query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .having('user_id', '=', getQueryBuilder(getQueryClient(connection)).from('accounts').select('id'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | havingNull', (group) => {
    ...
    test('add or having null clause', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            ...
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')['havingNull']('deleted_at')
            .orHavingNull('updated_at')
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        ...
        const { sql: resolverSql, bindings: resolverBindings } = db
            ...
            .toSQL();
        const { sql: knexResolverSql, bindings: knexResolverBindings } = connection.client
            .from('users')['havingNull']('my_deleted_at')
...
test.group('Query Builder | havingNotNull', (group) => {
    ...
    test('add or having not null clause', async ({ assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')
            .from('users')['havingNotNull']('deleted_at')
            .orHavingNotNull('updated_at')
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        ...
        const { sql: resolverSql, bindings: resolverBindings } = db
            ...
            .toSQL();
        const { sql: knexResolverSql, bindings: knexResolverBindings } = connection.client
            .from('users')['havingNotNull']('my_deleted_at')
...
test.group('Query Builder | havingExists', (group) => {
    ...
    test('add having exists clause', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            ...
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')['havingExists']((builder) => {
            builder.select('*').from('accounts').whereRaw('users.account_id = accounts.id');
        })
            .toSQL();
        assert.equal(sql, knexSql);
    ...
    test('add having exists clause as a subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            ...
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')['havingExists'](connection.client.select('*').from('accounts'))
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        await connection.disconnect();
...
test.group('Query Builder | havingNotExists', (group) => {
    ...
    test('add having not exists clause', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            ...
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')['havingNotExists']((builder) => {
            builder.select('*').from('accounts').whereRaw('users.account_id = accounts.id');
        })
            .toSQL();
        assert.equal(sql, knexSql);
    ...
    test('add having not exists clause as a subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            ...
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection.client
            .from('users')['havingNotExists'](connection.client.select('*').from('accounts'))
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        await connection.disconnect();
...
test.group('Query Builder | count', (group) => {
    ...
    test('count by raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .count(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .count({
...
test.group('Query Builder | countDistinct', (group) => {
    ...
    test('count by raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .countDistinct(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .countDistinct({
...
test.group('Query Builder | min', (group) => {
    ...
    test('use raw queries to compute min', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .min(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .min({
...
test.group('Query Builder | max', (group) => {
    ...
    test('use raw queries to compute max', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .max(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .max({
...
test.group('Query Builder | sum', (group) => {
    ...
    test('use raw queries to compute sum', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .sum(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .sum({
...
test.group('Query Builder | sumDistinct', (group) => {
    ...
    test('use raw queries to compute sumDistinct', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .sumDistinct(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .sumDistinct({
    ...
    test('use subqueries to compute sumDistinct', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .sumDistinct(getQueryBuilder(getQueryClient(connection)).where('is_verified', true).from('profiles'), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .sumDistinct({
...
test.group('Query Builder | avg', (group) => {
    ...
    test('use raw queries to compute avg', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .avg(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .avg({
...
test.group('Query Builder | avgDistinct', (group) => {
    ...
    test('use raw queries to compute avgDistinct', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .avgDistinct(getRawQueryBuilder(getQueryClient(connection), 'select * from profiles where is_verified = ?', [true]), 'u')
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .avgDistinct({
...
test.group('Query Builder | whereLike', (group) => {
    ...
    test('add where like clause as a raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereLike('age', getRawQueryBuilder(getQueryClient(connection), 'select min_age from ages limit 1;'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | whereILike', (group) => {
    ...
    test('add whereILike clause as a raw query', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereILike('age', getRawQueryBuilder(getQueryClient(connection), 'select min_age from ages limit 1;'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
    ...
    test('wrap raw query', async ({ assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .where((q) => q.whereILike('age', connection.client.raw('select min_age from ages limit 1;')))
            .where((q) => q.whereNotNull('deleted_at'))
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
...
test.group('Query Builder | whereJson', (group) => {
    ...
    test('define whereJson value as a callback', async ({ assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .whereJsonObject('location', (q) => q.from('locations').where('user_id', '1'))
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        db = getQueryBuilder(getQueryClient(connection));
        ...
        const { sql: knexResolverSql, bindings: knexResolverBindings } = connection
            .client.from('users')
            .whereJsonObject('my_location', (q) => q.from('locations').where('my_user_id', '1'))
            .toSQL();
        assert.equal(resolverSql, knexResolverSql);
        assert.deepEqual(resolverBindings, knexResolverBindings);
        await connection.disconnect();
    ...
    test('define whereJson value as a subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereJson('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
test.group('Query Builder | whereNotJson', (group) => {
    ...
    test('define whereNotJson value as a callback', async ({ assert }) => {
        ...
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
            .whereNotJsonObject('location', (q) => q.from('locations').where('user_id', '1'))
            .toSQL();
        assert.equal(sql, knexSql);
        assert.deepEqual(bindings, knexBindings);
        db = getQueryBuilder(getQueryClient(connection));
        ...
        const { sql: knexResolverSql, bindings: knexResolverBindings } = connection
            .client.from('users')
            .whereNotJsonObject('my_location', (q) => q.from('locations').where('my_user_id', '1'))
            .toSQL();
        assert.equal(resolverSql, knexResolverSql);
        assert.deepEqual(resolverBindings, knexResolverBindings);
        await connection.disconnect();
    ...
    test('define whereNotJson value as a subquery', async ({ assert }) => {
        ...
        const { sql, bindings } = db
            .from('users')
            .whereNotJson('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
            .toSQL();
        const { sql: knexSql, bindings: knexBindings } = connection
            .client.from('users')
...
if (['pg', 'mysql'].includes(process.env.DB)) {
    test.group('Query Builder | whereJsonSuperset', (group) => {
        ...
        test('define whereJsonSuperset value as a subquery', async ({ assert }) => {
            ...
            const { sql, bindings } = db
                .from('users')
                .whereJsonSuperset('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
                .toSQL();
            const { sql: knexSql, bindings: knexBindings } = connection
                .client.from('users')
            ...
            const { sql: knexResolverSql, bindings: knexResolverBindings } = connection
                .client.from('users')
                .whereJsonSupersetOf('my_location', connection.client.from('locations').where('user_id', '1'))
                .toSQL();
            assert.equal(resolverSql, knexResolverSql);
            assert.deepEqual(resolverBindings, knexResolverBindings);
            await connection.disconnect();
    ...
    test.group('Query Builder | whereNotJsonSuperset', (group) => {
        ...
        test('define whereNotJsonSuperset value as a subquery', async ({ assert }) => {
            ...
            const { sql, bindings } = db
                .from('users')
                .whereNotJsonSuperset('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
                .toSQL();
            const { sql: knexSql, bindings: knexBindings } = connection
                .client.from('users')
                .whereJsonNotSupersetOf('location', connection.client.from('locations').where('user_id', '1'))
                .toSQL();
            assert.equal(sql, knexSql);
            assert.deepEqual(bindings, knexBindings);
            db = getQueryBuilder(getQueryClient(connection));
            ...
            const { sql: knexResolverSql, bindings: knexResolverBindings } = connection
                .client.from('users')
                .whereJsonNotSupersetOf('my_location', connection.client.from('locations').where('user_id', '1'))
                .toSQL();
            assert.equal(resolverSql, knexResolverSql);
            assert.deepEqual(resolverBindings, knexResolverBindings);
            await connection.disconnect();
    ...
    test.group('Query Builder | whereJsonSubset', (group) => {
        ...
        test('define whereJsonSubset value as a subquery', async ({ assert }) => {
            ...
            const { sql, bindings } = db
                .from('users')
                .whereJsonSubset('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
                .toSQL();
            const { sql: knexSql, bindings: knexBindings } = connection
                .client.from('users')
    ...
    test.group('Query Builder | whereNotJsonSubset', (group) => {
        ...
        test('define whereNotJsonSubset value as a subquery', async ({ assert }) => {
            ...
            const { sql, bindings } = db
                .from('users')
                .whereNotJsonSubset('location', getQueryBuilder(getQueryClient(connection)).from('locations').where('user_id', '1'))
                .toSQL();
            const { sql: knexSql, bindings: knexBindings } = connection
                .client.from('users')
            ...
            const { sql: knexResolverSql, bindings: knexResolverBindings } = connection
                .client.from('users')
                .whereJsonNotSubsetOf('my_location', connection.client.from('locations').where('user_id', '1'))
                .toSQL();
            assert.equal(resolverSql, knexResolverSql);
            assert.deepEqual(resolverBindings, knexResolverBindings);
            await connection.disconnect();
